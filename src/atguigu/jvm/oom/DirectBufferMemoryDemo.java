 ...
 *  它可以使用Native函数库直接分配堆外内存，然后通过一个存储在Java堆里面的DirectByteBuffer对象作为这块
 *  内存的引用进行操作。这样能在一些场景中显著提高性能，因为避免了在Java堆和Native堆中来回复制数据。
 *
 *  ByteBuffer,allocate(capability) 第一种方式是分配JVM堆内存，属于GC管理范围，由于需要拷贝所以速度相对较慢
 *  ByteBuffer,allocateDirect(capability) 第二种方式是分配OS本地内存，不属于GC管理范围，由于不需要内存拷贝，
 *  所以速度相对较快。
 *
...
public class DirectBufferMemoryDemo {
    ...
    public static void main(String[] args) {
        ...
        //-XX:MaxDirectMemorySize=5m  我们配置5MB，但实际使用6MB，故意使坏
